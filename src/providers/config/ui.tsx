...
export default function ConfigPanel() {
	...
	const [saveStatus, setSaveStatus] = useState<{ [scope: string]: 'saved' | 'error' | null }>({})
						...
						onSubmit={async (e) => {
							...
							try {
								...
								for (const key of Object.keys(schema)) {
									...
									if (val !== (values[scope]?.[key] || '')) {
										await handleSave(scope, key, val)
									}
								}
					...
					>
						...
						<div className="space-y-4">
							{Object.entries(schema).map(([key, field]) => {
								const val = values[scope]?.[key] || field.default || ''
								return (
									<div key={key} className="flex flex-col gap-1.5">
										...
										<div className="relative flex-1">
											<input
												...
												defaultValue={val}
												placeholder={field.default ?? `Enter ${key}`}
						...
						<div className="mt-5 pt-4 border-t border-neutral-700 flex justify-end items-center gap-3">
							{saveStatus[scope] === 'saved' && (
							...
							<button
								type="submit"
								className="px-5 py-2 bg-neutral-700 hover:bg-neutral-600 text-white rounded font-medium transition-colors border border-neutral-600 cursor-pointer text-sm"
							>
								save
							</button>
						</div>
