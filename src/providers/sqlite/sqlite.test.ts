...
describe('SqliteProvider', () => {
	...
	beforeEach(() => {
		provider = SqliteProvider({ exoEval: (() => {}) as any, ring0: Database, config: { dataDir: testRoot } })
	})
	...
	it('scoped returns ScopedSqlite', () => {
		const scoped = provider.scoped('test')
	...
	it('different clients get isolated databases', () => {
		const a = provider.scoped('clientA')
		const b = provider.scoped('clientB')
	...
	it('persists across close/reopen', () => {
		const scoped = provider.scoped('persist')
		scoped.exec('CREATE TABLE t (val TEXT)')
		scoped.run('INSERT INTO t (val) VALUES (?)', ['hello'])
		provider.close()

		const provider2 = SqliteProvider({ exoEval: (() => {}) as any, ring0: Database, config: { dataDir: testRoot } })
		const scoped2 = provider2.scoped('persist')
